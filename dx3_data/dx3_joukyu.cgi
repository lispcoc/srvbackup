...

{
  ...
  sub is_valid {
      my $self = shift;
      if ($self->{droise} && $self->{droise} ne "Dロイス") {
        return 1;
      }
      else {
        return 0;
...
my $url  = "https://docs.google.com/spreadsheets/d/1JnCyspi76_ZVL7wgGLOVwP2gjJ65_NT16oF4zChwKfY/export?format=csv&gid=586078756";
my $file = "data_joukyu.csv";
...
while (my $row = $csv->getline($fh)) {
  my ($droise, $syndrome, $name, $type, $max_lv, $timing, $skill, $value, $difficulty, $target, $range, $limit, $effect) = @$row;
  ...
  if ($new_effect->is_valid) {
    if (!$effects{$droise}) {
      my @array;
      $effects{$droise} = \@array;
    }
    push (@{$effects{$droise}}, $new_effect) ;
  }
}
...
print "<a href=dx3_joukyu.cgi?view_mode=$view_mode&simple=$simple>すべて</a> / ";
foreach my $key_droise (sort keys (%effects)) {
  print "<a href=dx3_joukyu.cgi?key=$key_droise&view_mode=$view_mode&simple=$simple>$key_droise</a> / ";
}
print "<br>\n";
if ($simple eq "yes") {
  print "<b>";
}
print "<a href=dx3_joukyu.cgi?key=$key&view_mode=old&simple=yes>簡易表示</a>";
if ($simple eq "yes") {
  print "</b>";
}
print " / ";
...
print "<a href=dx3_joukyu.cgi?key=$key&view_mode=old&simple=no>通常表示</a>";
if ($simple ne "yes") {
  print "</b>";
}
...
foreach my $key_droise (sort keys (%effects)) {
  if ($key) {
    if ($key ne $key_droise) {
      next;
    }
  }
  @effects_in_droise = @{$effects{$key_droise}};
  print "<hr>\n";
  print "<h3><a name=$key_droise>$key_droise</a></h3>\n";
  if ($simple eq "yes") {
    print "<table class='type02'>\n";
    print "<tbody>\n";
  }
  foreach $effect (@effects_in_droise) {
    my $title_color = "#eee";
...
if ($csv_is_update) {
  generate_js (\%effects, "auto_joukyu.js");
}
